...
# (OBS: é possível (e mais "elegante") expecificar os modelos
# diretamente sem construir as variáveis no data.frame -- VER MAIS
# ABAIXO).
df <- transform(df,
                ic = ifelse(x < c, 0, 1),
                xp = ifelse(x < c, 0, x),
...
# Gráficos das regressões ajustadas. Aqui é construido data-frame para
# predição recriando as variáveis tal como foram fornecidas para ajuste.
...
# Lendo dados.
adf <- read.table("http://www.leg.ufpr.br/~paulojus/CE092/df02.txt",
                  head = TRUE)
...
amodseg$m0 <- lm(Y1 ~ x,
                 data = adf)
amodseg$m1 <- lm(Y1 ~ x + I((x - 1) * (x > 1)),
                 data = adf)
amodseg$m2 <- lm(Y1 ~ x + I((x - 0.7) * (x > 0.7)) +
                     I((x - 1.5) * (x > 1.5)),
                 data = adf)
